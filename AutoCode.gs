...
function applyLeagueCorruptionFix(applyChanges) {
  ...
  // ── 실제 복원 (applyChanges=true일 때만) ──
  // 진단과 100% 동일한 uniq 목록만 수정 → 진단=적용 보장.
  // league 칸(10) = correctLeague(=tr.from), points 칸(5) = fixedPts.
  // forward-freeze(commit 2f47210) 덕에 이후 48h 재보고가 다시 안 덮어씀.
  if (applyChanges) {
    uniq.forEach(function(h) {
      recordSheet.getRange(h.row, 10).setNumberFormat('@').setValue(h.correctLeague);
      recordSheet.getRange(h.row, 5).setValue(h.fixedPts);
    });
  }
  ...
  var head = applyChanges
    ? '[복원 적용 완료] ' + uniq.length + '건 수정 (멤버 ' + Object.keys(byMember).length + '명)'
    : '[손상 진단 — DRY RUN] 총 ' + uniq.length + '건 (멤버 ' + Object.keys(byMember).length + '명)';
  var out = head + '\n' + diag + '\n멤버별: ' + perMember + '\n\n' + lines.join('\n');
  Logger.log(out);
  return out;
}
...
// ⚠️ DEPRECATED & 비활성화 ──────────────────────────────
// 이 함수는 전체 history를 타임라인 재구성으로 비교해 215건 false positive를
// 만들어내는 치명적 버그가 있었습니다 (초기 리그 추정 불가 + timestamp Date
// 파싱 깨짐). 절대 사용 금지. 정확한 복원은 applyLeagueCorruptionFix(true).
function repairRecordLeagues() {
  throw new Error(
    'repairRecordLeagues는 폐기되었습니다 (215건 오복원 버그). ' +
    '진단: diagnoseLeagueCorruption() / 복원: applyLeagueCorruptionFix(true) 사용'
  );
}
...
function removeAllPrewarmTriggers() {
  ...
  var n = 0;
  for (var i = 0; i < t.length; i++) {
    if (t[i].getHandlerFunction() === 'prewarmDashboardCache_') {
      ScriptApp.deleteTrigger(t[i]); n++;
    }
  }
  return 'removed ' + n + ' prewarm trigger(s)';
}
...
var BOOTSTRAP_ANCHORS_ = [
  ...
  },
  {
    ...
    krw: 200000,
    note: '복제 비용 ≈ 주말 1회. 폐쇄 그룹만 사용, 외부 수요 미검증, 매출 0.'
  },
  {
    name: '동작하는 b2c/b2b 앱 — 검증된 반복 수요 0 (매출0·1곳 일회성·무료유저 소수 모두 여기)',
    ...
    krw: 1000000,
    note: '복제 비용 ≈ 며칠~2주. 일회성 거래·소수 무료 사용자는 시장 신호 아님. 코드값 거의 0, 아이디어·도메인 가치만. 대부분의 바이브코딩 결과물이 이 칸.'
  },
  {
    name: '복수 무관계 고객 반복 사용 + 초기 MRR 발생 (첫 PMF 신호, 소규모)',
    features: { service_type: 'b2b_saas', monetization: 'subscription', target_market: 'company', tech_complexity: 'medium', validation_stage: 'paying_users' },
    krw: 10000000,
    note: '★ 진짜 문턱. 서로 모르는 복수 고객이 자발적 반복 결제. 일회성/지인 거래는 여기 못 옴(#3 고정). #3 대비 10배 절벽.'
  },
  {
    name: '검증된 반복 구독 30+ 유료 고객',
    ...
    krw: 20000000,
    note: '월 구독 검증 완료. 30+ 유료 = 강한 PMF 신호. 반복 매출 안정.'
  },
  {
    name: '엔터프라이즈 SaaS (ARR 1억+, 기업 5+, 기술 해자)',
...
function handleEvalSubmit(params) {
  ...
  var systemPrompt =
    ...
    '페르소나 (반드시 차별화하여 평가):\n' +
    '- VC Vault (보수 엔터프라이즈): 매출 모델·유료 전환·운영 비용 중시. 가장 박하게.\n' +
    '- VC Rocket (얼리 그로스): 시장 잠재력·바이럴·UX 중시. 진짜 트랙션 보이면 가장 후하게.\n' +
    '- VC Forge (기술 시드): 코드 품질·엔지니어링 깊이·기술 해자 중시. 클론은 박하게.\n\n' +
    '═══════════════════════════════════════\n' +
    '★★★ 1순위 (평가의 핵심) — 실제 서비스를 직접 검증하라 ★★★\n' +
    '═══════════════════════════════════════\n' +
    '평가의 본질은 "사용자 설명이 얼마나 그럴듯한가"가 아니라\n' +
    '"실제 링크에 들어갔을 때 무엇이 진짜로 동작하는가"입니다.\n' +
    'user prompt에 GitHub 메타/README + 데모 HTML 실측 자료가 첨부됩니다. 이것이 1차 근거입니다.\n\n' +
    '반드시 다음을 실측 자료에서 직접 확인하고 평가에 반영:\n' +
    '1. 데모가 실제로 동작하는가? (HTTP 200 + 실제 콘텐츠 vs SPA 빈 shell / 로그인 게이트 / 데드링크)\n' +
    '2. 어떤 기능이 실제 구현돼 있는가? (HTML 본문·README에서 확인 가능한 실 기능 vs 말로만 존재)\n' +
    '3. 결제/구독이 실재하는가? (Stripe·Toss·결제 페이지 흔적 vs "유료 사용자 있다"는 말뿐)\n' +
    '4. 기술 깊이가 있는가? (README 구현 디테일 vs boilerplate / AI 프롬프트 단순 래퍼)\n' +
    '5. 활발히 개발·운영되는가? (최근 push·저장소 크기·stars vs 방치)\n\n' +
    '★ 복제 비용 테스트 (가장 중요한 잣대) ★\n' +
    '"숙련된 개발자가 AI를 끼고 이 결과물을 며칠~2주 안에 재현할 수 있는가?"\n' +
    '- YES (대부분의 바이브코딩 결과물) → 코드 자체 가치 ≈ 그 인건비(수십~수백만). 그 이상은\n' +
    '  오직 (검증된 반복 매출) or (검증된 데이터/네트워크 해자) or (대체불가 도메인 관계)로만 정당화.\n' +
    '- 위 3가지 중 검증된 게 없으면 → 100만원 안팎 이하. 화려한 설명은 가치가 아님.\n\n' +
    '신뢰성 디스카운트: 사용자 주장과 실측이 어긋날수록 더 박하게.\n' +
    '- "유료 사용자 N명/PMF" 주장인데 결제·콘텐츠 흔적 없음 → 5천원~30만원으로 추락 가능\n' +
    '- "기술 차별화" 주장인데 README가 단순 래퍼/boilerplate → 기술 점수 박탈\n' +
    '- 1곳 일회성 거래·지인 거래·소수 무료 사용자 = 시장 신호 아님 (매출 0과 동급 취급)\n' +
    '- 데모 죽었거나 GitHub fork/archived/private → 강한 디스카운트\n' +
    '실측이 주장을 뒷받침할 때만 정상가/후한 평가.\n\n' +
    '═══════════════════════════════════════\n' +
    '★ 2순위 (보조) — 앵커는 가격 밴드 sanity-check 용도일 뿐 ★\n' +
    '═══════════════════════════════════════\n' +
    '아래 앵커는 "이 정도 검증 수준이면 대략 이 밴드"를 가늠하는 보조 척도입니다.\n' +
    '앵커와의 정밀 비교(±몇%)에 매몰되지 말고, 1순위 실측 검증으로 가치를 먼저 정한 뒤\n' +
    '그 값이 어느 앵커 밴드에 들어가는지 sanity-check만 하세요. 실측 결론이 항상 우선합니다.\n\n' +
    anchorsText + '\n\n' +
    '핵심 밴드 구분:\n' +
    '- 검증된 반복 수요(복수 무관계 고객 반복 결제/MRR) 없음 → 1만~100만원 칸\n' +
    '- 그 문턱을 넘어야만 1,000만원+ (절벽임. 일회성·지인·무료유저는 못 넘음)\n\n' +
    '═══════════════════════════════════════\n' +
    '★ 평가 산출 규칙 ★\n' +
    '═══════════════════════════════════════\n' +
    '1. 절대 금지: 모든 VC가 동일 KRW. 페르소나에 따라 명확히 다르게 (VC 간 ±20%~50% 차이).\n' +
    '2. 절대 금지: 근거 없는 후한 평가. 의심스러우면 박하게 (false positive보다 false negative 선호).\n' +
    '3. 절대 상한: 5,000원 이상 ~ 300,000,000원 이하 정수만.\n\n' +
    '═══════════════════════════════════════\n' +
    '★ note/summary 작성 규칙 (사용자가 직접 보는 텍스트) ★\n' +
    '═══════════════════════════════════════\n' +
    '- 프로젝트 자체의 본질적 강점/약점만 (note 60자, summary 120자 이내).\n' +
    '- 절대 금지 단어: "앵커", "#1", "#2", "벤치마크", "대비 +N%", "대비 -N%", "비교 대상".\n' +
    '   ❌ "앵커 대비 -24%, 매출 모델 약함"   ✅ "결제 흔적 없어 매출 주장 미검증, 복제 용이"\n' +
    '- 실측에서 확인한 사실을 근거로 (예: "데모가 로그인 게이트라 기능 검증 불가").\n\n' +
    '═══════════════════════════════════════\n' +
    '★ 응답 형식 (JSON만, 다른 텍스트 금지) ★\n' +
    '═══════════════════════════════════════\n' +
    '{\n' +
    ' "evaluations":[\n' +
    '   {"vc":"VC Vault","krw":<int>,"note":"<실측 근거 기반 강약점, 앵커 언급 금지>"},\n' +
    '   {"vc":"VC Rocket","krw":<int>,"note":"..."},\n' +
    '   {"vc":"VC Forge","krw":<int>,"note":"..."}\n' +
    ' ],\n' +
    ' "summary":"<3 VC 결론·차이를 자연스럽게 종합, 앵커 언급 금지>"\n' +
    '}';
